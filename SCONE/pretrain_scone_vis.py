...
def save_train_params(save=False):
    ...
    params["n_view_max_for_scone_occ"] = 9
    ...
    model_name = "model_scone_vis"
    model_name = model_name + "_" + params["training_metric"]

    if params["occ_no_view_harmonics"]:
        model_name += "_occ_no_vh"

    if params["no_view_harmonics"]:
        model_name += "_no_view_harmonics"

    if params["surface_epsilon_is_constant"]:
        model_name += "_constant_epsilon"
    else:
        model_name += "_adaptative_epsilon"
    ...
    # model_name += "_seed" + str(params["random_seed"]) + "_" + str(params["torch_seed"])
    ...
    params["scone_vis_model_name"] = model_name

    # -----Json name to save params-----
    json_name = "train_params_" + params["scone_vis_model_name"] + ".json"
...
def run(ddp_rank=None, params=None):
    ...
    scone_vis, optimizer, opt_name, start_epoch, best_loss, best_coverage = initialize_scone_vis(params=params,
                                                                                                 scone_vis=scone_vis,
                                                                                                 device=device,
    ...
    for t_e in range(params.epochs):
        ...
        if is_master:
            ...
            }, "unvalidated_" + params.scone_vis_model_name + ".pth")
            ...
            if current_loss < best_train_loss:
                torch.save({
                    ...
                    'train_losses': train_losses,
                    'val_losses': val_losses,
                }, "best_unval_" + params.scone_vis_model_name + ".pth")
        ...
        if current_val_loss < best_loss:
            ...
            if is_master:
                torch.save({
                    ...
                    'train_losses': train_losses,
                    'val_losses': val_losses
                }, "validated_" + params.scone_vis_model_name + ".pth")
        ...
        if is_master and current_val_coverage > best_coverage:
            ...
            torch.save({
                ...
                'train_losses': train_losses,
                'val_losses': val_losses
            }, "coverage_validated_" + params.scone_vis_model_name + ".pth")
